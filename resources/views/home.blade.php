...
<div class="bg-emerald-700 text-white text-center py-20 rounded-lg shadow-lg mb-8 relative overflow-hidden">
    ...
    <a href="{{ route('shop.index') }}" class="mt-6 inline-block bg-amber-500 hover:bg-amber-600 text-white px-8 py-3 rounded-lg shadow-lg transition duration-300">تسوق الآن</a>
</div>

<div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">

    {{-- عروض خاصة (Splide Slider) --}}
  

  {{-- قسم البحث --}}
<div class="bg-white shadow-lg rounded-lg p-6 mb-8">
    <h2 class="text-2xl font-bold text-amber-700 mb-6">ابحث عن منتجاتك</h2>
    <form action="{{ route('products.search') }}" method="GET" class="flex flex-col md:flex-row md:items-center gap-4">
        <input 
            type="text" 
            name="query" 
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @foreach($products as $product)
                    <div class="bg-white rounded-lg p-4 shadow-md border border-gray-100 hover:shadow-lg transition-all duration-300 flex flex-col">
                        <img src="{{ $product->image ? asset('storage/' . $product->image) : asset('images/default-product.png') }}" alt="{{ $product->name }}" class="w-full h-48 object-cover rounded-lg">
                        ...
                        <div class="flex justify-between items-center mt-4 gap-2">
                            ...
                            <button onclick="openProductModal('{{ $product->id }}')" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center transition duration-300">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ml-1" viewBox="0 0 20 20" fill="currentColor">
