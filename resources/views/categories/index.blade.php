...
<div class="p-0 bg-white rounded-xl overflow-hidden shadow-2xl transform transition-all hover:shadow-2xl border border-emerald-100">
    ...
    <div class="bg-gradient-to-r from-emerald-600 via-emerald-500 to-teal-500 text-white py-6 px-8">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('categories.create') }}" class="bg-amber-500 hover:bg-amber-600 text-white px-5 py-2.5 rounded-lg text-sm font-medium flex items-center transition duration-300 transform hover:scale-105 shadow-md">
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-100">
                @foreach($categories as $category)
                    <tr class="hover:bg-emerald-50 transition duration-150 {{ $loop->odd ? 'bg-gray-50' : '' }}">
                        <td class="py-5 px-6 font-medium text-gray-800">{{ $category->name }}</td>
                        <td class="py-5 px-6">
