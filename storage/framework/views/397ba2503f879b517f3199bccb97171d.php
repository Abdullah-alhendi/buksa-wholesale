                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div class="md:col-span-1">
                            ...
                            <select name="category" id="category" class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 text-right bg-white text-gray-800 font-medium">
                                ...
                                <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($category->id); ?>" <?php echo e(request('category') == $category->id ? 'selected' : ''); ?> class="font-medium text-gray-800">
                        ...
                        <div class="md:col-span-1">
                            ...
                            <select name="sort" id="sort" class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 text-right bg-white text-gray-800 font-medium">
                                <option value="latest" <?php echo e(request('sort') == 'latest' ? 'selected' : ''); ?> class="font-medium text-gray-800">الأحدث</option>
                                <option value="price_asc" <?php echo e(request('sort') == 'price_asc' ? 'selected' : ''); ?> class="font-medium text-gray-800">السعر: من الأقل للأعلى</option>
                                <option value="price_desc" <?php echo e(request('sort') == 'price_desc' ? 'selected' : ''); ?> class="font-medium text-gray-800">السعر: من الأعلى للأقل</option>
                                <option value="popular" <?php echo e(request('sort') == 'popular' ? 'selected' : ''); ?> class="font-medium text-gray-800">الأكثر مبيعاً</option>
                                <option value="wholesale" <?php echo e(request('sort') == 'wholesale' ? 'selected' : ''); ?> class="font-medium text-gray-800">أفضل عروض الجملة</option>
                            </select>
                        </div>
...
<div class="container mx-auto px-4 mb-12">
        ...
        <?php $__empty_1 = true; $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div class="bg-white rounded-xl shadow-lg hover:shadow-xl transition duration-300 ease-in-out transform hover:-translate-y-2 overflow-hidden h-full flex flex-col">
                ...
                <div class="relative h-56 overflow-hidden">
                    <img src="<?php echo e($product->image ? asset('storage/' . $product->image) : asset('images/default-product.png')); ?>" 
    ...
    <?php if($product->wholesale_price < $product->price): ?>
        <?php echo e($product->wholesale_price); ?> د.أ
    <?php else: ?>
        <?php echo e($product->price); ?> د.أ
    <?php endif; ?>
</div>
                    ...
                    <?php if($product->discount > 0): ?>
                        <div class="absolute top-4 left-4 bg-gradient-to-r from-red-500 to-pink-500 text-white px-3 py-1 rounded-full text-sm font-bold shadow-lg">
                            خصم <?php echo e($product->discount); ?>%
                        </div>
                    <?php endif; ?>
                ...
                <div class="p-6 flex-grow flex flex-col">
                    <h5 class="text-xl font-bold text-gray-800 mb-3 line-clamp-2"><?php echo e($product->name); ?></h5>
                    <p class="text-gray-600 mb-4 flex-grow line-clamp-3"><?php echo e($product->description); ?></p>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <span class="text-lg font-bold text-green-600">
                                <?php echo e($product->wholesale_price); ?> د.أ
                                <small class="text-xs text-gray-500">/ <?php echo e($product->unit); ?></small>
                            </span>
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center">
                                <?php if($product->wholesale_price < $product->price): ?>
                                    <span class="line-through text-gray-400 text-sm mr-2"><?php echo e($product->price); ?> د.أ</span>
                                <?php endif; ?>
                                <span class="<?php echo e($product->wholesale_price < $product->price ? 'text-red-500' : 'text-gray-700'); ?> text-sm font-semibold">
                                    <?php echo e($product->price); ?> د.أ
                                    <small class="text-xs text-gray-500">/ <?php echo e($product->unit); ?></small>
                                </span>
                            </div>
                        ...
                        <?php if($product->wholesale_price < $product->price): ?>
                            <div class="mt-2 text-center">
                                <span class="inline-block bg-gradient-to-r from-green-500 to-blue-500 text-white text-xs px-2 py-1 rounded-full">
                                    وفر <?php echo e($product->price - $product->wholesale_price); ?> د.أ عند الشراء بالجملة
                                </span>
                            </div>
                        <?php endif; ?>
                        
                        <!-- الحد الأدنى لشراء الجملة -->
                        <?php if($product->min_wholesale_quantity): ?>
                            <div class="text-xs text-center text-gray-500 mt-1">
                                (الحد الأدنى لشراء الجملة: <?php echo e($product->min_wholesale_quantity); ?> <?php echo e($product->unit); ?>)
                            </div>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="flex justify-between items-center mb-3 text-sm">
                        ...
                        <span class="font-bold text-gray-800"><?php echo e($product->stock); ?> <?php echo e($product->unit); ?></span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-2 mb-4">
                        <div class="bg-gradient-to-r from-green-500 to-green-400 h-2 rounded-full transition-all duration-500" 
                             style="width: <?php echo e(min(100, ($product->stock / max(1, $product->max_stock)) * 100)); ?>%"></div>
                    </div>
                    
                    <!-- أزرار الإجراءات -->
                    <div class="mt-auto">
                        <form action="<?php echo e(route('cart.add', $product->id)); ?>" method="POST" class="add-to-cart-form mb-3">
                            <?php echo csrf_field(); ?>
                            <input type="hidden" name="is_wholesale" value="0">
                            <div class="flex gap-2">
                                <input type="number" name="quantity" value="<?php echo e($product->min_order); ?>" 
                                       min="<?php echo e($product->min_order); ?>" max="<?php echo e($product->stock); ?>" 
                        ...
                        <a href="<?php echo e(route('shop.show', $product->id)); ?>" class="w-full block text-center border border-blue-500 text-blue-500 hover:bg-blue-500 hover:text-white font-semibold py-2 px-4 rounded-lg transition duration-300 ease-in-out">
        ...
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
            <div class="col-span-full text-center py-16">
                <div class="max-w-md mx-auto">
                    <img src="<?php echo e(asset('images/empty-cart.svg')); ?>" alt="لا توجد منتجات" class="w-48 h-48 mx-auto mb-6 opacity-50">
                    <h4 class="text-2xl font-semibold text-gray-500 mb-4">لا توجد منتجات متاحة حالياً</h4>
                    <a href="<?php echo e(route('shop.index')); ?>" class="inline-block bg-green-600 hover:bg-green-700 text-white font-semibold py-3 px-6 rounded-lg transition duration-300 ease-in-out transform hover:scale-105">
                        عرض جميع المنتجات
                    </a>
                </div>
            </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\alhen\buksa-wholesale\resources\views/shop/index.blade.php ENDPATH**/ ?>